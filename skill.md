...
默认使用 `https://aivisn.com`。如果用户使用自建服务，设置 `CREATIVE_SERVER` 环境变量。

### 金币要求

创建任务会预扣金币（宫格视频 1800、RunningHub 200-2000、其他 10-500）。如果返回「金币不足」错误，引导用户去 aivisn.com 充值。
...
**输出说明：**
- 基础工具：`get_upload_url`、`confirm_upload`、`task_status`
...
| 用户需求 | 推荐模板类型 | 说明 |
|---------|-------------|------|
| 产品展示视频 | grid_video | 宫格分镜 + 线稿动画，适合电商产品 |
| 数字人播报 | digitalhuman | 虚拟人像 + 语音合成 |
| 图片转视频 | runninghub 相关 | 动作迁移、表情驱动等 |
| 自定义工作流 | raw | 灵活但需要用户了解 workflow 配置 |
| 多模型聚合 | omni | 调用多个 AI 模型 |
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CREATIVE_SERVER` | `https://aivisn.com` | Creative Server 地址 |
